...
export function parseCff(reader: Reader): CffTable {
	...
	for (const topDict of topDicts) {
		...
		if (topDict.fdSelect !== undefined) {
			...
			fdSelects.push(parseFDSelect(reader, charStrings[charStrings.length - 1]?.length ?? 0));
		} else {
 ...
 * Parse Private DICT
 */
function parsePrivateDict(reader: Reader, strings: string[]): PrivateDict {
	const dict = parseDict(reader);
